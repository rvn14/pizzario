...
export const PizzaCard: React.FC<{ pizza: Pizza }> = ({ pizza }) => {
    ...
    const handleOrder = () => {
        ...
        console.log(`Ordering ${pizza._id} ${quantity} ${pizza.name} (${selected}) for $${getPrice().toFixed(2)}`);
        //check if pizza is already in cart
        const existingItem = cartStore.items.find(item => item.id === pizza._id);
        if (existingItem) {
            // If it exists, update the quantity
            cartStore.updateItemQuantity(pizza._id, existingItem.quantity + quantity);
            return;
        }
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div>
                            ...
                            <div className='flex items-center gap-2'>
                                <Button
                                    onClick={remove}
                                    className='bg-white/10 hover:bg-white/20 text-white rounded-full text-sm flex items-center justify-center cursor-pointer w-8 h-8 p-0'
